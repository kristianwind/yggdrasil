...
| `euro-office.yaml` | ghcr.io/euro-office/documentserver | 80 | document server for OpenCloud (ONLYOFFICE-based, WOPI) — **its own server with its own subdomain**; put its address in the OpenCloud server's `OFFICE_URL`. Runs as root with its own postgres/rabbit/redis inside |
