...
| `opencloud.yaml` | opencloudeu/opencloud | 9200 | files/sync-and-share, one container, no database. Admin password is set at INSTALL only (blank = generated into the install log); reset later with `docker exec -it ygg-<id> opencloud idm resetpassword -u admin`. Needs an **https** address; TLS mode follows the domain automatically (self-signed cert when reached directly, plain http behind a panel domain, since NPM and the CF tunnel both speak http to the origin) |
| `euro-office.yaml` | ghcr.io/euro-office/documentserver | 80 | document server for OpenCloud (ONLYOFFICE-based, WOPI) — **its own server with its own subdomain**; put its address in the OpenCloud server's `OFFICE_URL`. Runs as root with its own postgres/rabbit/redis inside, but its services drop to `ds` — the install step opens the data dir to it. Admin panel is a separate opt-in (`ADMINPANEL_ENABLED`), off by default |
