...
| `GET` | `/api/servers/{id}/routes` | `server.view` | Every public hostname this server answers on, primary first, plus the TCP ports a hostname may be pointed at. The primary lives on the server row; the rest are separate records |
| `POST` | `/api/servers/{id}/routes` | Admin | Add an extra hostname, optionally naming which declared TCP port it targets (empty = the automatic choice: the port called `web`, else the first TCP one). Rejected if the hostname is already claimed on this panel, or if it is a bare label and no base domain is configured. Provisioned immediately when the server is running, otherwise on next start |
| `DELETE` | `/api/servers/{id}/routes/{routeID}` | Admin | Remove an extra hostname. The live proxy rule and DNS record are deleted **before** the record is forgotten — the other order loses the only note of what was provisioned |
...
| `GET` | `/api/settings/analytics` | Admin | The operator's own `<head>` snippet, and the origins it caused to be allowed. Empty on an install that has never set one — the panel ships no analytics of its own |
| `PUT` | `/api/settings/analytics` | Admin | Store or clear it. The panel derives the origins from the tag's `src`/`href` attributes and widens `script-src`, `connect-src` and `img-src` by **exactly those** — a strict CSP would otherwise block a third-party tag before it loaded. Inline `<script>` code is refused: allowing it means `'unsafe-inline'` on every page forever. Max 4 KB. Audited as `settings.analytics`, recording the origins rather than the markup |
