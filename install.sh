...
# --- In-panel self-update (decoupled) -------------------------------------
# The panel service is hardened (NoNewPrivileges + ProtectSystem), so it cannot
# escalate itself. Instead the panel drops a version tag into a request file in
# its state dir; a systemd path unit runs the root oneshot updater (its own
# UNsandboxed unit) which downloads + checksum-verifies the official release,
# replaces the binary and restarts the panel. No sudoers rule needed.
log "Installing self-update service..."
UPDATE_HELPER="/usr/local/bin/yggdrasil-update"
rm -f /etc/sudoers.d/yggdrasil-update  # remove the old sudoers approach if present
if curl -fsSL -o "$UPDATE_HELPER.new" "https://raw.githubusercontent.com/$REPO/main/deploy/yggdrasil-update.sh" 2>/dev/null \
  ...
  for unit in yggdrasil-update.service yggdrasil-update.path; do
    curl -fsSL -o "/etc/systemd/system/$unit" "https://raw.githubusercontent.com/$REPO/main/deploy/$unit" 2>/dev/null || \
      warn "Could not fetch $unit; in-panel updates disabled."
  done
else
  rm -f "$UPDATE_HELPER.new"
  warn "Could not fetch the update helper; in-panel updates disabled until the next install.sh run."
fi

systemctl daemon-reload
[ -f /etc/systemd/system/yggdrasil-update.path ] && systemctl enable --now yggdrasil-update.path >/dev/null 2>&1 || true
