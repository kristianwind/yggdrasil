...
func migrate(db *sql.DB) error {
	...
	// An HTTP path the panel requests to decide whether the APP is alive, as
	// opposed to whether its port is bound. Empty = off.
	//
	// The difference is not academic. Docker's userland proxy binds a published
	// port whether or not anything is listening behind it, so a container whose
	// app never started still answers a TCP connect — which is exactly what the
	// readiness check dials. Measured: a VPN-gated torrent client whose tunnel
