<script>
  ...
  // The backup nudge asks the operator to go and configure something. On a public
  // demo the reader is not the operator and cannot act on it — "set up a backup"
  // is a dead end there, sitting above the thing they came to look at. Hidden
  // rather than dismissible, because a first impression only happens once and a
  // visitor should not have to tidy up before seeing the product.
